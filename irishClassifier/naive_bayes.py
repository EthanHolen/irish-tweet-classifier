...
def tokenize(message):
    ...
    patt = re.compile(ur"[a-z0-9áéíóú'-]+", re.UNICODE)
...
def word_probabilities(counts, total_true, total_false, k=0.1):
...
def load_labeled_data():
    ...
    with codecs.open('nonulster.txt','r','utf-8') as file:
        for line in file:
            data.append((line, True))

    with codecs.open('ulster.txt','r','utf-8') as file:
        for line in file:
...
def classify_test_set(classifier):
    ...
    print 'id,label'
    with codecs.open('test.tsv','r','utf-8') as file:
        for line in file:
            pieces = line.split("\t")
